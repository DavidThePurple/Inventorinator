...
images and captured label images—is sent to the hosted or self-hosted Supabase
instance chosen by the user. Authentication and refresh tokens are stored on
the device so it can reconnect. A plain HTTP server address is supported for
