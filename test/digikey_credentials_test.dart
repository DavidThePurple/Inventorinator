    ...
    () async {
      ...
      final service = SupabaseSyncService(
        ...
        client: MockClient((request) async {
          ...
          expect(
            request.url.path,
            endsWith('set_inventorinator_digikey_credentials'),
          );
          expect(body['target_client_secret'], writes++ == 0 ? 'secret' : null);
          return http.Response('', 204);
        }),
      );
      final credentials = await service.getDigiKeyCredentials(session);
      expect(credentials.clientSecret, 'secret');
      await service.setDigiKeyCredentials(session, credentials);
      await service.setDigiKeyCredentials(session, const DigiKeyCredentials());
      expect(writes, 2);
    },
  );
  test(
    ...
    () async {
      ...
      const value = DigiKeyCredentials(
        clientId: 'fixture-id',
        clientSecret: 'fixture-secret-never-export',
      );
      DigiKeyCredentialStore(db, 'workspace-a').save(value, pending: true);
      db.close();
      db = await LocalDatabase.open(overridePath: path);
      try {
        final store = DigiKeyCredentialStore(db, 'workspace-a');
        expect(store.read()!.credentials.clientSecret, value.clientSecret);
        expect(store.read()!.pending, isTrue);
        expect(DigiKeyCredentialStore(db, 'workspace-b').read(), isNull);
        final exported = await db.exportPortableDatabase();
        expect(latin1.decode(exported), isNot(contains(value.clientSecret)));
        final old = store.read()!;
        store.save(const DigiKeyCredentials(), pending: true);
