...
class MouserSearchDialog extends StatefulWidget {
  const MouserSearchDialog({super.key, this.searchOverride});
  final Future<MouserPage> Function(String query, int offset)? searchOverride;
  @override
  State<MouserSearchDialog> createState() => _MouserSearchDialogState();
}

class _MouserSearchDialogState extends State<MouserSearchDialog> {
  final _query = TextEditingController();
  MouserClient? _client;
  MouserPage? _page;
  bool _busy = false;
  String? _error;
  String _lastQuery = '';
  final _statusKey = MouserSettings.statusKey;
  @override
  void dispose() {
    _query.dispose();
    _client?.close();
  ...
  Future<void> _search({int offset = 0}) async {
    ...
    try {
      ...
      _client ??= MouserClient(apiKey: MouserSettings.apiKey);
      ...
      if (mounted) {
        setState(
          () => _error = error is MouserException
              ? error.message
              : 'Could not read Mouser results. Please try again.',
        );
      }
    } finally {
  ...
  bool get _canSearch =>
      !_busy && (widget.searchOverride != null || MouserSettings.configured);

  Widget _searchControls(double width, double textScale) {
    final field = TextField(
      key: const Key('mouser-query'),
      controller: _query,
      enabled: !_busy,
      maxLength: 250,
  ...
  Widget _partCard(MouserPart part, double width) => SizedBox(
    width: width,
    child: Card(
      key: ValueKey('mouser-part-${part.supplierPartNumber}'),
        ...
        builder: (context, constraints) {
          ...
          return Column(
            ...
            children: [
              ...
              Expanded(
                child: ListView(
                  key: const Key('mouser-content'),
                  padding: EdgeInsets.fromLTRB(padding, 16, padding, 16),
                  keyboardDismissBehavior:
                      ScrollViewKeyboardDismissBehavior.onDrag,
                  children: [
                    if (widget.searchOverride == null &&
                        !MouserSettings.configured)
                      const Padding(
                        padding: EdgeInsets.only(bottom: 12),
                        child: Text(
                          'Set up Mouser in Remote Settings → Mouser first.',
                        ),
                      ),
