# Update an existing Inventorinator server from v21 to v24
...
```sh
umask 077
docker exec supabase-db pg_dump -U postgres -d postgres -Fc > inventorinator-before-v24.dump
```
...
Extract `inventorinator-server-v24.tar.gz` and run from the extracted folder:
...
Expect `24`. The connector log should report `Inventorinator schema v24 is ready`.
After the next successful app sync, the Supabase LED should turn green. Open
Remote Settings and use each optional feature's Sync control to retry locally
retained role templates/credentials; they are not part of the inventory outbox.

Until updated, the new client supports inventory sync on v21–v23 and shows an
