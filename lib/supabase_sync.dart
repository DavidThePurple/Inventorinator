...
class SupabaseSyncService {
  ...
  Future<List<ScratchPadNote>> listRemovedDeviceNotes(
    ...
    final result = await _rpc(
      session,
      'list_inventorinator_removed_device_notes',
      {'target_workspace': config.workspaceId},
    );
    return result is List
        ? result
              .whereType<Map>()
              .map(
                (row) => ScratchPadNote.fromRemoteJson(
  ...
  Future<List<ScratchPadNote>> listSharedScratchPadNotes(
    SupabaseSession session,
  ) async {
    final result = await _rpc(
      session,
      'list_inventorinator_shared_device_notes',
      {'target_workspace': config.workspaceId},
    );
    return result is List
        ? result
