...
- The bottom bar's Bulk Import button imports CSV, XLSX or JSON, opens Rapidizer, and exports CSV, XLSX or versioned portable JSON (which also carries kits and the shopping list). Spreadsheet imports start with a column-matching preview, every import offers to skip or keep possible duplicates, and item IDs survive an export and re-import.
