...
begin
  ...
  if member_role = 'builder' then
    ...
    if exists (
      select 1
      from jsonb_array_elements(coalesce(next_state->'builds', '[]'::jsonb)) build
    ...
    if exists (
      ...
      where not exists (
        select 1
        from jsonb_array_elements(coalesce(next_state->'inventory', '[]'::jsonb)) item
    ...
    if exists (
      with old_allocations as (
        select consumed.inventory_id,
               sum(greatest(least(
                 coalesce((line->>'usedQuantity')::numeric, 0) - (consumed.position - 1),
                 1::numeric
               ), 0::numeric)) as amount
        from jsonb_array_elements(coalesce(previous_state->'builds', '[]'::jsonb)) build
        cross join jsonb_array_elements(coalesce(build->'lines', '[]'::jsonb)) line
      ...
      old_inventory as (
        select item->>'id' as inventory_id,
               coalesce((item->>'quantity')::numeric, 1) as quantity
        from jsonb_array_elements(coalesce(previous_state->'inventory', '[]'::jsonb)) item
      ),
      new_inventory as (
        select item->>'id' as inventory_id,
               coalesce((item->>'quantity')::numeric, 1) as quantity
...
insert into public.inventorinator_schema(singleton, version) values(true, 7)
