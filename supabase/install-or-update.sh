...
release_tag=${INVENTORINATOR_RELEASE_TAG:-v0.1.1-alpha.2}
required_version=34
...
download_migrations() {
  ...
  migration_names='001_workshop_state.sql 002_anonymous_workspaces.sql 003_device_roles.sql 004_owner_devices.sql 005_team_roles_audit.sql 006_shared_build_permissions.sql 007_builder_quantity_integrity.sql 008_owner_recovery.sql 009_owner_durability.sql 010_owner_resilience.sql 011_device_role_management.sql 012_incremental_entity_sync.sql 013_preserve_legacy_locations.sql 014_authenticated_entity_read_policy.sql 015_disable_legacy_snapshot_writes.sql 016_remote_offline_purge_policy.sql 017_stable_device_repair.sql 018_reload_remote_purge_rpc.sql 019_allow_spool_usage_entity_sync.sql 020_incremental_sync_rpc_timeout.sql 021_fast_incremental_entity_writes.sql 022_role_templates.sql 023_digikey_credentials.sql 024_mouser_credentials.sql 025_custom_role_assignments.sql 026_device_scratchpad_notes.sql 027_shared_scratchpad_notes.sql 028_scratchpad_note_subjects.sql 029_delete_tombstone_precedence.sql 030_guarded_import_undo.sql 031_scratchpad_device_recovery.sql 032_filament_drying_policy.sql 033_inventory_modified_time.sql 034_checkout_sync.sql'
...
required_migration=$(find "$migrations_dir" -maxdepth 1 -type f \
  -name "$(printf '%03d' "$required_version")_*.sql" -print -quit)
if [ -z "$required_migration" ]; then
  echo "This updater is missing the required v$required_version migration." >&2
  echo 'Replace the complete Supabase bundle, then run ./apply-migrations.sh again.' >&2
  exit 1
fi
